...
namespace BrickPluginModels.Services
{
    ...
    public class VoidnessCalculator
    {
        ...
        public double CalculateMaxPossibleVoidness(double length, double width, double height,
        ...
        public double CalculateMinPossibleVoidness(double length, double width, double height,
            double holeRadius)
        {
        ...
        public VoidnessCalculationResult CalculateOptimalParameters(double length, double width,
            double height, double targetVoidness, HoleDistributionType distributionType)
        {
            if (targetVoidness > MaxVoidnessPercent || targetVoidness <= 0)
            {
                ...
                {
                    Success = false,
                    ErrorMessage = $"Пустотность должна быть в диапазоне (0, {MaxVoidnessPercent}]%"
                };
            }
            ...
            double voidnessMaxRadius = Math.Sqrt(MaxVoidnessPercent / 100.0 * length * width / Math.PI);
            ...
            for (double radius = MinSearchRadius; radius <= maxRadius; radius += RadiusSearchStep)
            {
                int maxHoles = distributionType == HoleDistributionType.Straight
                    ? _distributionCalculator.CalculateMaxHolesStraight(length, width, radius)
                    : _distributionCalculator.CalculateMaxHolesStaggered(length, width, radius);
                ...
                {
                    double voidness = CalculateCurrentVoidness(length, width, height, radius, holes);
            ...
            if (bestResult == null)
            {
                ...
                {
                    Success = false,
                    ErrorMessage = "Не удалось подобрать параметры для заданной пустотности"
                };
            }
        ...
        /// <summary>
        /// Рассчитывает количество отверстий для достижения заданной пустотности с фиксированным радиусом.
        /// </summary>
        /// <param name="length">Длина кирпича.</param>
        /// <param name="width">Ширина кирпича.</param>
        /// <param name="height">Высота кирпича.</param>
        ...
        /// <param name="distributionType">Тип распределения отверстий.</param>
        /// <returns>Результат расчета.</returns>
        public VoidnessCalculationResult CalculateHolesCountForVoidness(double length, double width,
            double height, double holeRadius, double targetVoidness, HoleDistributionType distributionType)
        {
            if (targetVoidness > MaxVoidnessPercent || targetVoidness <= 0)
            {
                return new VoidnessCalculationResult
            ...
            double actualVoidness = CalculateCurrentVoidness(length, width, height, holeRadius, calculatedHoles);
            ...
            if (actualVoidness > MaxVoidnessPercent)
            {
                ...
                {
                    Success = false,
                    ErrorMessage = $"Достижимая пустотность ({actualVoidness:F2}%) превышает максимум ({MaxVoidnessPercent}%)"
                };
            }
        ...
        public (double min, double max) GetVoidnessRange(double length, double width, double height,
            double holeRadius, HoleDistributionType distributionType)
        {
            double min = CalculateMinPossibleVoidness(length, width, height, holeRadius);
            double max = CalculateMaxPossibleVoidness(length, width, height, holeRadius, distributionType);
