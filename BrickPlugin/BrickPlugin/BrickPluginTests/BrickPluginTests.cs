...
namespace BrickPlugin.Tests
{
    ...
    public class BrickParametersTests
    {
        ...
        [TestMethod]
        public void CalculateDependent_RaisesMaxRadiusChangedEvent()
        {
            bool eventRaised = false;
            string eventHint = null;
            _parameters.MaxRadiusChanged += (s, h) => { eventRaised = true; eventHint = h; };
            _parameters[ParameterType.Width] = 200;
            Assert.IsTrue(eventRaised);
            Assert.AreEqual("Макс: 50.0 мм", eventHint);
        }
        ...
        {
            _parameters[ParameterType.Width] = 200;
            Assert.AreEqual("Макс: 50.0 мм", _parameters.GetMaxRadiusHint());
        }
